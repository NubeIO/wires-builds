...
class HistoryUtils {
    ...
    static addHistorySettingsConfig(self, valueInput = 0, isOutput = true) {
        if (!self.settingConfigs.hasOwnProperty('groups') &&
            !self.settingConfigs.hasOwnProperty('conditions'))
    ...
    static doHistoryFunctions(self) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (self.getInputData(self.histTriggerInput) && self.inputs[self.histTriggerInput].updated)
                yield HistoryUtils.storeLogEntry(self, input);
            else if (input != null && (yield HistoryUtils.checkCOV(self, input))) {
                yield HistoryUtils.storeLogEntry(self, input);
            }
            if (self.obj.length > 0)
                yield HistoryUtils.trySendStoredData(self);
            self.setOutputData(self.storedHistCountOutput, self.obj.length);
        });
    }
    static trySendStoredData(self) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (self.settings['databaseType'].value == 0) {
                var multiPointPost = [];
                const clientIDfromPlat = yield HistoryUtils.getClientID(self);
                const deviceIDfromPlat = yield HistoryUtils.getDeviceID(self);
            ...
            if (self.settings['databaseType'].value == 1) {
                ...
                var client = new Influx.InfluxDB({
                    ...
                    password: self.settings['authentication'].value
                        ? crypto_utils_1.default.decrypt(self.settings['password'].value)
                        : '',
                });
    ...
    static historyFunctionsForAfterSettingsChange(self, pointName = 'undefined') {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            yield HistoryUtils.changeAlarmsCount(self, self.settings['alarms_count'].value);
            yield HistoryUtils.renameAlarmInputs(self);
            yield HistoryUtils.changeTagsCount(self, self.settings['tags_count'].value);
            HistoryUtils.setPeriodicLogging(self);
            HistoryUtils.persistProperties(self);
            if (self.properties['isOutput'])
                self.onInputUpdated();
        });
    }
    static storeLogEntry(self, input) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            msg['tags'] = tags;
            self.obj.push(msg);
            while (self.obj.length > storageLimit) {
                self.obj.shift();
            }
            ...
            if (!self.settings['historyMode'].value) {
                ...
                if (input !== self.properties['lastHistoryValue']) {
                    if (typeof self.properties['lastHistoryValue'] !== 'number' ||
                        !isNumber ||
                        (isNumber &&
                            Math.abs(input - self.properties['lastHistoryValue']) >=
    ...
    static getDeviceID(self) {
        return new Promise((resolve, reject) => {
            try {
                HistoryUtils.findMainContainer(self).db.getNodeType('system/platform', (err, docs) => {
                    if (!err) {
                        let output = [];
                        output.push(docs);
    ...
    static findMainContainer(self) {
        if (self.hasOwnProperty('container'))
            return HistoryUtils.findMainContainer(self.container);
        else
            return self;
    }
}
