...
const logger = require('logplease').create('bacnet');
...
class BACnetDevice extends BACnetObject {
    constructor(deviceInfo, hostInfo = {}, nodeId = null, broadcastFuncAfterWrite) {
        ...
        this.client = null;
        this.nodeId = nodeId;
        this.broadcastFuncAfterWrite = broadcastFuncAfterWrite;
    }
    ...
    onWhoIs(msg) {
        ...
        if (msg.payload.lowLimit === undefined ||
    ...
    onWriteProperty(data) {
        ...
        logger.debug(`PRIORITY_ARRAY: ${PI.PRIORITY_ARRAY}`);
        ...
        if (data.payload.value.property.index === 0xffffffff) {
            ...
            logger.debug(priorityValue);
            logger.debug(priorityNum);
            logger.debug(priorityArr);
            logger.debug(arr);
            ...
            this.broadcastFuncAfterWrite(this.nodeId, data.payload.objectId.type, data.payload.objectId.instance, priorityValue, priorityNum, arr);
        }
        else {
    ...
    onReadProperty(msg) {
        ...
        if (msg.payload.property.index === 0xffffffff) {
            ...
            logger.debug(JSON.stringify(slot));
            logger.debug('333333');
            if (!slot) {
