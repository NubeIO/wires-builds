...
class Client extends EventEmitter {
  ...
  _handleNpdu(buffer, offset, msgLength, header) {
    // Check data length
    if (msgLength <= 0) return trace('No NPDU data -> Drop package');
    // Parse baNpdu header
    const result = baNpdu.decode(buffer, offset);
    if (!result) return trace('Received invalid NPDU header -> Drop package');
    ...
    if (msgLength <= 0) return trace('No APDU data -> Drop package');
  ...
  _receiveData(buffer, remoteAddress) {
    // Check data length
    if (buffer.length < baBvlc.BVLC_HEADER_LENGTH) return trace('Received invalid data -> Drop package');
    // Parse BVLC header
    const result = baBvlc.decode(buffer, 0);
    if (!result) return trace('Received invalid BVLC header -> Drop package');
    let header = {
    ...
    switch (result.func) {
      ...
      case baEnum.BvlcResultPurpose.REGISTER_FOREIGN_DEVICE:
        ...
        if (!decodeResult) return trace('Received invalid registerForeignDevice message');
